...
1. **Получение данных**: ESP32 публикует JSON в топик `esp32/sensors` брокера Mosquitto на хосте.
   ...
   ```
2. **Обработка**: `climat_mqtt_listener` принимает сообщение, извлекает поле `device` и использует его как `device_id` для вставки в таблицу `sensor_data` через `psycopg2`.
...
- Важно: скрипт использует поле `device` из входящего JSON как `device_id` для базы данных.  
  Реализован обработчик `on_disconnect`, автоматически восстанавливающий подключение к брокеру при обрывах.
...
**Обратите внимание:** фактическое имя колонки – `device_id`, а не `sensor_id`.
...
| `500 Internal Server Error` на `/api/*` | `climat_web` не может разрешить имя `db` или в SQL-запросах используется неверное имя колонки `sensor_id` вместо `device_id` | Убедитесь, что сервис `db` в сети `app-network`. Проверьте код API – везде должно быть `device_id`. |
| Предупреждение слушателя: `Missing 'device'` | В JSON-сообщении отсутствует поле `device` | Настройте ESP32 на отправку `"device": "esp32_1"`. При необходимости подкорректируйте `listener.py`. |
...
| Нет новых данных в графиках, логи слушателя остановились после определённой даты | Соединение с MQTT оборвалось, а в скрипте отсутствует логика переподключения | Обновите `listener.py`, добавив обработчик `on_disconnect` с циклом переподключения, и пересоберите контейнер. |
| `/api/data` возвращает пустой массив, хотя `/api/latest` показывает данные | Слишком короткий запрошенный период (например, `1h`) при старых записях, либо неверный параметр `device` | Используйте период `7d` или `all`. Проверьте, что передаётся `device=esp32_1` и что SQL-запросы фильтруют по `device_id`. |

...
├── listener.py                 # MQTT Listener (с авто-переподключением)
└── .dockerignore
```
...
* 🔄 Автоматическое восстановление подключения MQTT-слушателя при обрывах
...
* Колонка с идентификатором устройства называется `device_id` (в документации ранее ошибочно упоминалась `sensor_id`).
...
| ----- | --------------------------------------- | ---------------------------- |
| `GET` | `/api/latest`                           | Последние показания (любое устройство) |
| `GET` | `/api/stats`                            | Общая статистика по всем устройствам |
| `GET` | `/api/data?period=1h&device=esp32_1`   | Исторические данные          |
| `GET` | `/api/health`                           | Проверка состояния сервиса   |

### Параметры `/api/data`

* `period` — `1h`, `6h`, `24h`, `7d`, `30d`, `all`
* `device` — идентификатор устройства (`esp32_1` по умолчанию)
...
| 500 Internal Server Error | Нет подключения к БД, или в SQL-запросах используется `sensor_id` вместо `device_id` | Проверить Docker-сеть и исправить SQL |
| Нет новых данных, логи слушателя остановились | Обрыв MQTT-соединения без переподключения | Обновить `listener.py` с обработчиком `on_disconnect` и пересобрать контейнер |
| `/api/data` возвращает пустой массив, но `/api/latest` работает | Неверный `device` (например, `esp32_main`) или слишком короткий период | Используйте `device=esp32_1` и период `7d` или `all` |
| Заняты порты 80/443       | Другой веб-сервер использует порты  | Использовать системный Nginx, внутренний прокси пробросить на `127.0.0.1:8080` |
| Таблицы отсутствуют       | SQL-инициализация не выполнилась    | Пересоздать том БД (`docker compose down -v`) |
